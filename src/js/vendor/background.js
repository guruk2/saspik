...
headerResponsiveBreakpoint = 1024;
...
(function($) {
	"use strict";
	var ps_saaspik = window.ps_saaspik || {};
	
	window.ps_saaspik = ps_saaspik;
	
	ps_saaspik.window = $(window),
	ps_saaspik.document = $(document),
	ps_saaspik.windowHeight = ps_saaspik.window.height(),
	ps_saaspik.windowWidth = ps_saaspik.window.width(),
	ps_saaspik.scrollbarWidth = 0,
	ps_saaspik.windowScrollTop = 0;
	ps_saaspik.sameOrigin = true;
	
	ps_saaspik.initObjectsSizing = function() {
		try {
			ps_saaspik.sameOrigin = window.parent.location.host == window.location.host;
		} catch (e) {
			ps_saaspik.sameOrigin = false;
		}
		
		var recalcWindowOffset = function() {
			ps_saaspik.windowScrollTop = ps_saaspik.window.scrollTop();
		};

		ps_saaspik.document.ready(function() {
			...
			ps_saaspik.scrollbarWidth = div.offsetWidth - div.clientWidth;
		...
		var recalcWindowInitHeight = function() {
			ps_saaspik.windowHeight = ps_saaspik.window.height();
			ps_saaspik.windowWidth = ps_saaspik.window.width() + ps_saaspik.scrollbarWidth;
		...
		ps_saaspik.window.on("load", function() {
			$('body').trigger('reinit-waypoint');
		});
	};
	
	ps_saaspik.initParallaxBackground = function (){
		var psParallax = function() {
			$('.ps_stun_header_vertical_parallax, .ps-row-parallax, .ps-column-parallax, .ps-fade-on-scroll, .ps-row-bg-image.ps_vertical_parallax, .ps-row-bg-image.ps_horizontal_parallax, .ps-multi-parallax-layer, .stuning-header-inner .page-title-inner').each(function() {
				
				var $self = $(this), offsetCoords, topOffset, selfHeight;

				var recalcInitValues = function() {
					offsetCoords = $self.offset();
					if($self.hasClass('ps_vertical_parallax')) {
				...
				ps_saaspik.window.on("load resize", recalcInitValues);
				...
				ps_saaspik.window.on("load scroll", function() {
					if(!mobileEnable && Modernizr.touch && ps_saaspik.windowWidth < 768) {
						return;
					}
					if (( (ps_saaspik.windowScrollTop + ps_saaspik.windowHeight) > topOffset ) && ( (topOffset + selfHeight) > ps_saaspik.windowScrollTop )) {
						recalcInitValues();

						var diff = (topOffset - ps_saaspik.windowScrollTop) / 3,
						diffPos = -(diff * speed),
						starPosition = '50% 50%';

						if ($self.data('parallax_offset')) {
							if($self.hasClass('ps_vertical_parallax') || $self.hasClass('ps_horizontal_parallax')) {
								if($self.hasClass('ps_vertical_parallax')) {
									starPosition = '50% calc(50% + '+$self.data('parallax_offset')+'px)';
								} else if($self.hasClass('ps_horizontal_parallax')) {
						...
						if($self.hasClass('ps_horizontal_parallax')) {
							coords = diffPos + 'px,' + statPos;
						}

						if($self.hasClass('ps-multi-parallax-layer')) {
						...
						if($self.hasClass('ps-row-parallax')) {
							var yPos = -(diff * speed);
						...
						if($self.hasClass('ps-column-parallax')) {
						// Move the column
						var yPos = -(diff * speed);
					...
					if($self.hasClass('ps_stun_header_vertical_parallax')) {
						// Move the bg container
						var yPos = Math.floor(ps_saaspik.windowScrollTop * speed / 5);
					...
					if($self.hasClass('ps-fade-on-scroll')) {
						var height = $self.height();

						// Fade the row
						$self.css({
							opacity: (1 + 1/(height/(topOffset - ps_saaspik.windowScrollTop)))
						});
					}
					if(	$self.hasClass('ps_vertical_parallax') || $self.hasClass('ps_horizontal_parallax') ||	$self.hasClass('ps-multi-parallax-layer')) {
						window.requestAnimationFrame(function() {
							$self.css({
...
var initMobileBgImage = function() {
	$('.ps-row-bg-image').each(function() {
		...
		if(defaultImage != '' && mobileImage != '') {
			if(typeof ps_saaspik.windowWidth != 'undefined' && ps_saaspik.windowWidth < resolution && mobileImage) {
...
ps_saaspik.window.on('load resize', initMobileBgImage);

if (!$('html').is('.lt-ie10, .lt-ie9, .lt-ie8')) {
	psParallax();
	ps_saaspik.window.load(function(){
		psParallax();
		if (!Modernizr.touch && ps_saaspik.windowWidth > 768) {
...
ps_saaspik.fullHeightRow = function() {
	var psFullHeightRow = function () {
		$( '.ps-row-full-height:first' ).each( function () {
			var offset,
			fullHeight,
			$self = $(this);
			...
			setTimeout(function() {
				...
				if($('.ps-frame-line.line-bottom').length > 0) {
					offset += $('.ps-frame-line.line-bottom').height();
				}
				if ( offset < ps_saaspik.windowHeight ) {
					fullHeight = ps_saaspik.windowHeight - offset;
	...
	ps_saaspik.window.on('load resize', psFullHeightRow);
};

ps_saaspik.initEqualHeights = function() {
	var eqHeightInit = function() {
		var w = ps_saaspik.windowWidth;
		$('.vc-row-wrapper.equal-height-columns').each(function(){
			var $container = $(this),
			resolution = $container.data('resolution') ? $container.data('resolution') : 768,
		...
		$('.ps-equal-height-wrapper').each(function(){
			if($(this).hasClass('ps-mobile-destroy-equal-heights')) {
				if (w>768) {
	...
	ps_saaspik.document.ready(function() {
		$('.vc-row-wrapper.equal-height-columns.aligh-content-verticaly').each(function(){
			...
			$columns.each(function() {
				$(this).wrapInner('<div class="ps-vertical-aligned"></div>');
			});
		});
	});
...
ps_saaspik.initVideoBg = function() {
	$('.ps-video-bg video, .ps-video-bg .ps-bg-frame').each(function() {
		var $self = $(this),
		ratio = 1.778,
		pWidth,
		pHeight,
		...
		$self.parents('.ps-video-bg').siblings('.ps-video-controller').unbind('click').on('click', function(e) {
			e.preventDefault();
			var $button = $(this);
			if($button.hasClass('ps-socicon-ic_pause_48px')) {
				$self.get(0).pause();
				$button.removeClass('ps-socicon-ic_pause_48px').addClass('ps-socicon-icon-play');
			} else {
				$self.get(0).play();
				$button.removeClass('ps-socicon-icon-play').addClass('ps-socicon-ic_pause_48px');
			}
		});
		$self.parents('.ps-video-bg').siblings('.ps-sound-controller').unbind('click').on('click', function(e) {
			e.preventDefault();
			var $button = $(this);
			if($button.hasClass('ps-socicon-unmute')) {
				$self.prop('muted',false);
				$button.removeClass('ps-socicon-unmute').addClass('ps-socicon-mute');
			} else {
				$self.prop('muted',true);
				$button.removeClass('ps-socicon-mute').addClass('ps-socicon-unmute');
			}
		});
		setSizes();
		ps_saaspik.window.on('load resize', function() {
			setSizes();
		});
	...
	if($('.ps-youtube-bg').length > 0) {
		...
		window.onYouTubeIframeAPIReady = function() {
			$('.ps-youtube-bg iframe').each(function() {
				var $self = $(this),
				id = $self.attr('id');
	...
	if($('.ps-vimeo-bg').length > 0) {
		ps_saaspik.document.ready(function() {
			$('.ps-vimeo-bg iframe').each(function() {
				var $self = $(this);
...
ps_saaspik.initAnimatedBg = function() {
	ps_saaspik.document.ready(function() {
		$('.ps-row-bg-image.ps_animated_bg').each(function() {
			var $self = $(this),
			...
			if(!mobileEnabled && Modernizr.touch && ps_saaspik.windowWidth < 768) {
				return;
			}
...
ps_saaspik.initCanvasBg = function() {
	var init = function() {

		$('.ps-row-bg-canvas').each(function(){
			var $self = $(this);
			//if($self.data('mobile-disable') && $self.data('mobile-disable') == 'on' && ps_saaspik.windowWidth < 1100) {
				if(ps_saaspik.windowWidth < 1100) {
					return false;
				}
...
ps_saaspik.init = function() {
	// ps_saaspik.initObjectsSizing();		
	ps_saaspik.initVideoBg();	
	ps_saaspik.fullHeightRow();		
	ps_saaspik.initEqualHeights();
	ps_saaspik.initParallaxBackground();
	ps_saaspik.initAnimatedBg();
	ps_saaspik.initCanvasBg();
	//ps_saaspik.initMousemoveParallax();
